...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!parsed.success) {
    res.status(400).json({ error: "Please enter the invite code your school gave you." });
    return;
  }
  const code = parsed.data.code.toUpperCase();

  if (!ctx.schoolId) {
    res.status(400).json({ error: "Select your school before entering an invite code." });
    return;
  }
  ...
  try {
    const result = await db.runTransaction(async (tx) => {
      ...
      if (!inviteSnap.exists) {
        throw new UserFacingError("That code doesn't look right. Double-check it and try again.");
      }
      ...
      if (invite.role !== ctx.role) {
        throw new UserFacingError(`That code is for a ${invite.role} account, not a ${ctx.role} account.`);
      }
      ...
      for (const classRef of classRefsToClaim) tx.update(classRef, { teacherId: ctx.uid });

      return { userPatch };
    });

    await writeAuditLog(ctx, { action: "write:redeem_invite", result: "success", args: { role: ctx.role } });
    res.status(200).json({ success: true, linked: result.userPatch });
  } catch (err) {
    if (err instanceof UserFacingError) {
      await writeAuditLog(ctx, { action: "write:redeem_invite", result: "denied", reason: err.message });
      res.status(400).json({ error: err.message });
      return;
    }
    console.error("redeem-invite failed", err);
    await writeAuditLog(ctx, { action: "write:redeem_invite", result: "error" });
